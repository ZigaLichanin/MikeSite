...
//Go through values and search database
for($i=0;$i<count($search);$i++){
$getthreadsbysearch="select * from forumposts where title like '%$search[$i]%'";
//Get result into array
$result=mysqli_query($dbcon,$getthreadsbysearch);
//Check for error
...
if(!isset($rows)){
	echo"<script>function showerror(){
	 ...
	 nothreads.innerHTML+='<li><p style=\'font-size:150%\; text-align:center\;\'>No threads to show! Try searching something else!</p></li>';
	 }
...
//Go through values and search database
for($i=0;$i<count($search);$i++){
$getthreadsbysearch="select * from forumposts where title like '%$search[$i]%' ORDER by forumposts.DateAdded DESC,forumposts.TimeAdded desc";
//Get result into array
$result=mysqli_query($dbcon,$getthreadsbysearch);
//Check for error
if(mysqli_error($dbcon)!=null){echo mysqli_error($dbcon);}
